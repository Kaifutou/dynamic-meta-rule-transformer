...
\end{defbox}
\Explain 这说明设想并不要求抛弃 Transformer，而是允许在保留基本结构的同时，让注意力投影矩阵、前馈层或门控参数受到当前记忆状态的调制。
...
\Explain 这个命题很重要，因为它把整个想法变成了一个标准的“有内部状态的动力系统”。一旦写成这个形式，就可以进一步讨论稳定性、可塑性、可控性和收敛行为。
...
\Explain 这是“永久性记忆”最简单的数学雏形：重复且被验证的经验，会通过巩固机制把长期记忆逐渐拉向该经验本身。
